...
public class TestConfig {
    ...
    private final static HashMap<String, Organizaton> sampleOrgs = new HashMap<>();
...
//                    DEFAULT_CONFIG));
            ...
            for (Map.Entry<Object, Object> x : sdkProperties.entrySet()) {
                ...
                if (key.startsWith(INTEGRATIONTESTS_ORG)) {
                    ...
                    if (match.matches() && match.groupCount() == 1) {
                        ...
                        sampleOrgs.put(orgName, new Organizaton(orgName, val.trim()));

                    }
                }
            }

            for (Map.Entry<String, Organizaton> org : sampleOrgs.entrySet()) {
                final Organizaton sampleOrg = org.getValue();
    ...
    public Collection<Organizaton> getIntegrationTestsSampleOrgs() {
        return Collections.unmodifiableCollection(sampleOrgs.values());
    }

    public Organizaton getIntegrationTestsSampleOrg(String name) {
    ...
    private Properties getEndPointProperties(final String type, final String name) {
        ...
        File cert = Paths.get("config", "crypto-config/ordererOrganizations".replace("orderer", type), domainName, type +"s",
